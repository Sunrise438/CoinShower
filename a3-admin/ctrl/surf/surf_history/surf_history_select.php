...
if (isset($_GET['t'])){
    if ($_GET['t'] == 'today'){
        ...
        $nsql = "SELECT COUNT(DISTINCT uname) AS COUNTemail FROM surf_history WHERE date > CURDATE() ";
        $sql = "SELECT uname, COUNT(id), SUM(amount) FROM surf_history WHERE date > CURDATE()"
        ...
        $nsql = "SELECT COUNT(id) FROM surf_history WHERE uname = '$uname' ";
        $sql = "SELECT * FROM surf_history WHERE uname = '$uname' "
                . "ORDER BY $order_type $order LIMIT $page,$page_limit";
        
    } else {
        $nsql = "SELECT COUNT(id) FROM surf_history";
        $sql = "SELECT * FROM surf_history ORDER BY $order_type $order LIMIT $page,$page_limit";
    }
