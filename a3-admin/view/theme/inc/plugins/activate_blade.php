<div class="row">
    <div class="col-xl-12 mt-3 mb-3 p-5 border border-2 rounded" id="uploadpluginsblade">
        ...
        ?>
        <h5 class="mb-3"><?php echo ucwords($pluginsName.' '.$nameName.' : '.test_input($_GET['activate'])); ?></h5>
        <h6><?php echo ucfirst($IfYouHaveaPluginsActivationKeyName); ?></h6>
        <div class="row">
            <div class="col-xl-6">
                <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]).'?activate=upwall';?>" enctype="multipart/form-data">
    
                    <input type="hidden" name="plugins_activation" value="1" required="">
                    <input type="hidden" name="plugins" value="<?php echo test_input($_GET['activate']); ?>" required="">
                    ...
                    <div class="input-group mb-3 mt-3">
                        ...
                        <button class="btn btn-outline-success fw-bold"><?php echo ucwords($activateName.' '.$pluginsName);?></button>
                    </div>
